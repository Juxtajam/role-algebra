...
"""Phase 6 Item B — the Item A AGGREGATE attention diagnostic applied to the
8-LAYER fine-tune finals, per the Item B reporting clause:
"aggregate attention diagnostic from Item A applied to any seed where
composed crosses 0.5."
...
app = modal.App("dv3-itemB-aggregate-attention8")
...
def diagnose(seeds: list):
    ...
    p("=" * 88)
    p("PHASE 6 ITEM B — ITEM A AGGREGATE DIAGNOSTIC ON THE 8-LAYER FINALS")
    p(f"seeds probed (composed crossed 0.5 during fine-tuning): {seeds}")
    p(
    ...
    for seed in seeds:
        ...
        model.eval()
        p(
            f"\n--- 8L finetune final seed{seed} (step {ck['step']}; composed "
            f"final {fr['final']['held_episode']:.3f}, max "
        ...
        res[f"seed{seed}"]["composed_Q_P"] = comp
    ...
    for seed in seeds:
        for label, _ in QTYPES:
            ...
            flags[(seed, label)] = verdict
            res.setdefault("adjudication", {})[f"seed{seed}|{label}"] = dict(
                verdict=verdict,
                direct_x=direct_x,
                comp_x=comp_x,
                pair_x=pair_x,
    ...
    with open("/results/checks/itemB_aggregate_attention8.txt", "w") as f:
        f.write("\n".join(out) + "\n")
    progress.save_json("checks/itemB_aggregate_attention8.json", res)
    log("MILESTONE: ITEM B AGGREGATE ATTENTION DIAGNOSTIC (8L) done — " + overall)
    vol.commit()
    return "\n".join(out)


@app.local_entrypoint()
def main(seeds: str = "2"):
    print(diagnose.remote([int(s) for s in seeds.split(",")]))
