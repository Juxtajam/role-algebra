"""Track B induction pretraining — Phase 10.
...
Usage:
    modal run --detach phase10/trackB/induction_pretrain_modal.py
"""

import modal
import pathlib

app = modal.App("dv3-trackB-induction-pretrain")
...
# These guard against any unintended change to the corpus definition or model source.
...
# Reference batch at seed 9999 (deterministic numpy RNG — pure-data check).
...
def pretrain(seed: int) -> dict:
    ...
    key = f"phase10/trackB/induction/pretrain/seed{seed}"
    ckpt_path = progress.results_dir() / f"{key}/ckpt.pt"
    ckpt_path.parent.mkdir(parents=True, exist_ok=True)

    BATCH, EVAL_EVERY, WARMUP = 256, 500, 500

    # ---------- corpus hash verification (requirement 1) ----------
    log(f"TrackB pretrain seed{seed}: verifying v2 corpus hashes...")

    # Check induction.py source integrity
    ...
    ), f"induction.py hash mismatch: {induction_hash} != {V2_INDUCTION_SOURCE_SHA256}"

    # Check model.py source integrity
    with open("/root/dv3/trained/model.py", "rb") as f:
        model_hash = hashlib.sha256(f.read()).hexdigest()
    assert (
        model_hash == V2_MODEL_SOURCE_SHA256
    ), f"model.py hash mismatch: {model_hash} != {V2_MODEL_SOURCE_SHA256}"

    # Check reference batch — pure numpy, no torch needed
    ...
    assert (
        ref_hash == V2_REFERENCE_BATCH_SHA256
    ), f"reference batch hash mismatch: {ref_hash} != {V2_REFERENCE_BATCH_SHA256}"
    log(
        f"TrackB pretrain seed{seed}: corpus hash verification PASSED "
        f"(induction.py, model.py, reference batch all match)"
    )
    ...
    log(
        f"TrackB pretrain seed{seed}: tied-name round-trip verified ({rt:.3f}), "
        f"8 layers, d_model=128, 4 heads, d_ff=512, fp32"
    )
    ...
    log(
        f"TrackB pretrain seed{seed}: START — induction corpus v2, "
        f"batch {BATCH}, AdamW lr 1e-3 wd 0.01 (name rows wd=0.0, frozen), "
        f"cosine/{MAX_STEPS} warmup {WARMUP}, hard {MAX_STEPS}"
    )

    rng = np.random.default_rng((seed, 77))
    ev_seqs = I.build_eval_set(seed)  # held-out, disjoint rng stream (seed, 55501)
    ...
    if ckpt_path.exists():
        ...
        log(f"TrackB pretrain seed{seed}: RESUMED at step {step}")
    ...
    try:
        while step < MAX_STEPS:
            ...
            if step % EVAL_EVERY == 0 or step == MAX_STEPS:
                ...
                log(
                    f"TrackB pretrain seed{seed}: step {step} loss {rec['loss']:.4f} "
                ...
                if not frozen_ok:
                    log(
                        f"MILESTONE: TrackB pretrain seed{seed} FREEZE VIOLATION "
                        f"at step {step} — frozen name rows not bit-exact; run invalid"
                    )
        ...
        # ========== FULL VERIFICATION BATTERY (requirement 4) ==========
        log(
            f"MILESTONE: TrackB pretrain seed{seed} finished {MAX_STEPS} steps; "
            f"running full verification battery..."
        )

        # 4a. Standard behavioural + mechanistic verification
        ver = I.verify_induction(model, ev_seqs, device)

        # 4b. Off-period content probe at periods 11, 13, 40
        log(
            f"TrackB pretrain seed{seed}: running off-period content probe "
            f"(periods 11, 13, 40)..."
        )
        off_period_masses = {}
        off_period_accs = {}
        for L_test in [11, 13, 40]:
            # Build eval set with exact prefix length L_test
            rng_op = np.random.default_rng((seed, 70000 + L_test))
            # Generate sequences with fixed L by replicating sample_batch logic
            # but forcing Ls = L_test
            r = rng_op.random((1024, I.VOCAB - 1))
        ...
        log(
            f"MILESTONE: TrackB pretrain seed{seed} DONE at {step} — "
        ...
        if not ver["pass"]:
            log(
                f"MILESTONE: TrackB pretrain seed{seed} {status} — checkpoint "
        ...
        log(
            f"MILESTONE: TrackB pretrain seed{seed} CRASHED:\n" + traceback.format_exc()
        )
        crash_ver = {
            "seed": seed,
            "status": "CRASHED",
...
def finalize_summary(all_results_json: str):
    ...
    summary = {
        ...
        "passing_threshold_met": len(passing) >= 10,
        "per_seed_details": {},
    }
    for r in all_results:
        s = r.get("seed", "?")
        summary["per_seed_details"][str(s)] = {
    ...
    path = progress.save_json("phase10/trackB/induction/pretrain/summary.json", summary)
    vol.commit()
    print(f"\nMILESTONE: TrackB induction pretrain SUMMARY written to {path}")
    print(
        f"  {len(passing)}/{N_SEEDS} passed, {len(failing)} failed, "
        f"{len(crashed)} crashed"
    )
    if summary["passing_threshold_met"]:
        print("  >10 passing seeds — threshold MET")
    else:
        print("  WARNING: fewer than 10 passing seeds — need replacements")
    return summary
...
def main():
    ...
    # Launch all seeds in ONE .map() call — required by Modal --detach
    # (only the *last* triggered function survives after parent disconnects).
    # Modal handles A10G concurrency automatically (default limit typically 10).
    seeds = list(range(N_SEEDS))
    log(f"\n{'='*60}")
    log(f"TrackB pretrain: launching ALL {N_SEEDS} seeds {seeds}")
    log(f"{'='*60}")
    all_results = list(pretrain.map(seeds))
    log(f"All {len(all_results)} seeds complete")

    # Assemble summary via a Modal CPU function (writes to volume)
    log("\n" + "=" * 60)
    log("All seeds complete — writing summary...")
