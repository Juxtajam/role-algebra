...
"""Phase 7 Item 1 steps 2+3 — task fine-tuning of the
verified 8-LAYER seeds 3-7, protocol byte-identical to Phase 6 Item B
(phase6_finetune8.py). ONE declared, artifact-only addition (required by
the Item 1 reporting clause "aggregate attention diagnostic ... on the
final checkpoint AND one mid-formation checkpoint" — Phase 6 overwrote
ckpt.pt every eval, so no mid-formation checkpoint survived for seed 2;
that retroactive gap is noted in the Item 1 report): at the
FIRST eval where composed held-out crosses 0.5, the model state is
snapshotted to ckpt_midformation.pt. No training-affecting change of any
kind: same rng streams, same batch order, same eval sets, same
hyperparameters, same gating.

Original Phase 6 docstring follows.
...
app = modal.App("dv3-phase7-finetune8")
        ...
        while step < MAX_STEPS:
            ...
            if step % EVAL_EVERY == 0 or step == MAX_STEPS:
                ...
                if comp_held >= 0.5 and not comp_05_logged:
                    ...
                    # Phase 7 declared artifact-only addition: mid-formation
                    # snapshot for the Item 1 second-hop diagnostic
                    torch.save(
                        dict(
                            model=model.state_dict(), step=step, composed_held=comp_held
                        ),
                        ckpt_path.parent / "ckpt_midformation.pt",
                    )
                    log(
                        f"phase6 finetune8 {key}: mid-formation checkpoint "
                        f"saved at step {step} (composed {comp_held:.3f})"
                    )
