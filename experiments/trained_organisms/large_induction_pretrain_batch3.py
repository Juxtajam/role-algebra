"""Track B induction pretraining — BATCH 3.
Seeds 11,13, both Arm R (VOCAB=3154) and Arm C (VOCAB=878) = 4 runs.
...
Usage:
    modal run --detach phase10/trackB/induction_pretrain_batch3.py
"""

import modal
import pathlib

app = modal.App("dv3-trackB-batch3")
...
# BATCH 3: seeds 11, 13, both arms  (seed 12 excluded — failed induction verification)
SEEDS = [11, 13]
ARMS = {
...
# These are placeholder values; MUST be recomputed before launch.
...
def pretrain(seed: int, arm: str) -> dict:
    ...
    log(
        f"TrackB batch3 {arm} seed{seed}: verifying v2 corpus hashes (VOCAB={vocab})..."
    )
    ...
    if arm_ref_hash != "PLACEHOLDER_RECOMPUTE_BEFORE_LAUNCH":
        ...
        log(f"TrackB batch3 {arm} seed{seed}: corpus hash verification PASSED")
    else:
        log(
            f"TrackB batch3 {arm} seed{seed}: SKIPPING reference batch hash "
    ...
    log(
        f"TrackB batch3 {arm} seed{seed}: tied-name round-trip verified ({rt:.3f}), "
    ...
    log(
        f"TrackB batch3 {arm} seed{seed}: START — induction corpus v2, "
    ...
    if ckpt_path.exists():
        ...
        log(f"TrackB batch3 {arm} seed{seed}: RESUMED at step {step}")
    ...
    try:
        while step < MAX_STEPS:
            ...
            if step % EVAL_EVERY == 0 or step == MAX_STEPS:
                ...
                log(
                    f"TrackB batch3 {arm} seed{seed}: step {step} loss {rec['loss']:.4f} "
                ...
                if not frozen_ok:
                    log(
                        f"MILESTONE: TrackB batch3 {arm} seed{seed} FREEZE VIOLATION "
                        f"at step {step}"
                    )
        ...
        log(
            f"MILESTONE: TrackB batch3 {arm} seed{seed} finished {MAX_STEPS} steps; "
        ...
        log(
            f"TrackB batch3 {arm} seed{seed}: running off-period content probe "
            f"(periods 11, 13, 40)..."
        )
        off_period_masses = {}
        off_period_accs = {}
        for L_test in [11, 13, 40]:
            ...
            log(
                f"TrackB batch3 {arm} seed{seed}: off-period L={L_test} "
        ...
        log(
            f"MILESTONE: TrackB batch3 {arm} seed{seed} DONE at {step} — "
        ...
        if not ver["pass"]:
            log(
                f"MILESTONE: TrackB batch3 {arm} seed{seed} {status} — checkpoint "
        ...
        log(
            f"MILESTONE: TrackB batch3 {arm} seed{seed} CRASHED:\n"
...
def finalize_summary(all_results_json: str):
    ...
    path = progress.save_json("phase10/trackB/induction/batch3/summary.json", summary)
    vol.commit()
    print(f"\nMILESTONE: TrackB batch3 induction pretrain SUMMARY written to {path}")
    print(
...
def main():
    ...
    # Build seed-arm pairs: seeds 11,13 × both arms = 4 runs
    runs = [(s, a) for s in SEEDS for a in ARMS]

    log(f"\n{'='*60}")
    log(
        f"TrackB BATCH 3: launching {len(runs)} runs "
    ...
    log("\n" + "=" * 60)
    log("Batch 3 complete — writing summary...")
