...
Model (approved fallback; Llama-3.3 token lacks scopes):
  ...
  @ 6f6073b423013f6a7d4d9f39144961bfbfbc386b, bf16, ungated,
...
Committed config: phase10/trackA2/committed_config.json, sha256
...
Session order (hard-coded, per committed_config.json):
  ...
  2. GATE BATTERY FIRST, all 4 path x vocab cells on the byte-frozen
     ...
     written to dv3-results:phase10/trackA2/gate/ and committed BEFORE any
     caching.
  ...
  4. If all cells pass: cache resid_post fp16 at the answer position, ALL
     ...
     dv3-results:phase10/trackA2/acts/; then disc-set behavioural labels
...
APP = modal.App("dv3-phase10-trackA2-gate")
...
CONFIG_SHA = "b6fa9dce222b3abb907e668e916f6187c2477639388367d72dc98e16de54fa30"
...
def verify_committed_config():
    fp = f"{RES}/phase10/trackA2/committed_config.json"
    h = sha_file(fp)
...
def run_session():
    ...
    os.makedirs(f"{RES}/phase10/trackA2/gate", exist_ok=True)
    ...
    def write_episode_csv(cell_name, recs, preds):
        fp = f"{RES}/phase10/trackA2/gate/episodes_gate_{cell_name}.csv"
    ...
    for path, vocab in CELLS:
        ...
        with open(
            f"{RES}/phase10/trackA2/gate/preds_gate_{path}_{vocab}.json", "w"
        ) as f:
            json.dump(preds, f)
    ...
    with open(f"{RES}/phase10/trackA2/gate/gate_results.json", "w") as f:
        json.dump(results, f, indent=2)
    vol.commit()
    log(
        f"GATE all-cells-met-thresholds: {all_pass} "
        f"(gate results + preds committed to phase10/trackA2/gate/)"
    )
    ...
    if not all_pass:
        ...
        with open(f"{RES}/phase10/trackA2/session_meta.json", "w") as f:
    ...
    os.makedirs(f"{RES}/phase10/trackA2/acts", exist_ok=True)
    for kind in ("gate", "disc"):
        for path, vocab in CELLS:
            recs = load_cell(kind, path, vocab)
            op = f"{RES}/phase10/trackA2/acts/{kind}_{path}_{vocab}.npy"
            cache_acts(recs, op)
    ...
    for path, vocab in CELLS:
        ...
        acc, strict, cons = orbit_metrics(recs, preds)
        with open(
            f"{RES}/phase10/trackA2/gate/preds_disc_{path}_{vocab}.json", "w"
        ) as f:
            json.dump(preds, f)
        log(
    ...
    checksums = {}
    for fn in sorted(os.listdir(f"{RES}/phase10/trackA2/acts")):
        if not fn.endswith(".npy"):
            continue
        fp = f"{RES}/phase10/trackA2/acts/{fn}"
        h1 = sha_file(fp)
        ...
        del arr
    with open(f"{RES}/phase10/trackA2/acts/checksums.json", "w") as f:
        json.dump(
            dict(
                content_hash=h,
                committed_config_sha=hc,
