"""Phase 10 Track A — targeted error-base diagnostic, Llama-3.3-70B ONLY.
Identical to Nemotron script. Single session, one model.
...
APP = modal.App("dv3-phase10-diag2-ll33")
...
def log(msg):
    print(f"[{time.strftime('%H:%M:%S')}] diag2-ll33: {msg}", flush=True)
...
def run_session():
    ...
    with open(f"{RES}/{OUT_DIR}/episodes_llama33.jsonl", "w") as f:
    ...
    with open(f"{RES}/{OUT_DIR}/summary_llama33.json", "w") as f:
